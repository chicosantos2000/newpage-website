...
<html lang="pt-pt">
  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
    ...
    <div class ="container">
      <div class="row">
        <div class="col-sm">
          <h1>Gestão de Noticias do autor</h1>
            <?php 
             include "sql_connect.php";
            
            if(isset($_COOKIE["login"]))
            {
                echo "Bem vindo, Autor";
                echo "<p><a href=\"logout.php\" class=\"btn btn-danger\">Sair</a></p>"; 
            }else{
            ...
            $login=$_COOKIE["login"];
            $sql_query="SELECT autor_id FROM autor WHERE autor_login='$login'";
            $resultado_c1=$ligacao->query($sql_query);
              if($resultado_c1->num_rows>0){
                    while($rowc1=$resultado_c1->fetch_assoc()){
                            $idautor=$rowc1["autor_id"];                     
                    }
                }
...
?> 
      ...
      <div class="row"> 
          ...
          <h3>Noticias</h3>
            <ul>
                <?php
                $sql_c="SELECT * FROM noticia WHERE noticia_autor = $idautor";
                $resultado_c=$ligacao->query($sql_c);
                
                if($resultado_c->num_rows>0){
                    while($rowc=$resultado_c->fetch_assoc()){
                        echo "<li class=\"list-group-item\"> <a class=\"btn btn-info\" href=\"editarnoticia.php?id=". $rowc["noticia_id"] . "\">" . $rowc["noticia_titulo"] . "</a></li>";
