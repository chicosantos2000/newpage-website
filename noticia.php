      ...
      <div class="row"> 
        <div class="col-sm-8">
          <h2>Produtos</h2>
            ...
            $id = $_GET["id"]; 
            $sql_p = "SELECT autor.autor_nome, categoria.categoria_nome, noticia.noticia_id, noticia.noticia_titulo, noticia.noticia_intro, noticia.noticia_autor, noticia.noticia_desenv, noticia.noticia_data, noticia.noticia_foto FROM ((noticia INNER JOIN autor ON noticia.noticia_autor=autor.autor_id) INNER JOIN categoria ON noticia.noticia_cat=categoria.categoria_id) WHERE autor.autor_id=$id";
            $resultado_p = $ligacao->query($sql_p);
            if ($resultado_p->num_rows > 0)
            { 
                while ($row = $resultado_p->fetch_assoc()){
                echo "<div class=\"card\">
                <div class=\"card-header\"> ". $row ["noticia_titulo"] ."</div>
                <div class=\"card-body\">
                <p><img src=\"fotos/" . $row["noticia_foto"] . "\" width=150px></p>
        ...
        <div class="col-sm-4"> 
          <h3>Categorias</h3>
            <ul>
                ...
                if($resultado_c->num_rows>0){
                    while($rowc=$resultado_c->fetch_assoc()){
                        echo "<li class=\"list-group-item\"> <a href=\"categoria.php?id=". $rowc["categoria_id"] . "\">" . $rowc["categoria_noticia"] . "</a></li>";
           ...
           <?php
            ...
            if($resultado_c->num_rows>0){
            while($rowc=$resultado_c->fetch_assoc()){
            echo "<li class=\"list-group-item\"> <a href=\"autor.php?id=". $rowc["autor_id"] . "\">" . $rowc["autor_nome"] . "</a></li>";
